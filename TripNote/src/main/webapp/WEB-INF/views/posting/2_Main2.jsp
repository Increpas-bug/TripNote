			...
			<c:forEach var="posting" items="${postingList}" varStatus="stat">
				...
				<form action="" id="frm_common_${posting.posting_no}">
					<input type="hidden" name="user_no"
						value="${sessionScope.member.user_no}"> <input
						type="hidden" name="posting_no" value="${posting.posting_no}">
				</form>
				<div class="col-sm-4">
					<!-- 포스팅 화면 띄우기 -->
					<article class="panel panel-default">
						<div class="panel-body">
							...
							<div>
								...
								<c:if test="${not empty posting.posting_uploadpath}">
									...
									<a data-toggle="modal"
										data-target="#photoView_${posting.posting_no}"> <img
										alt="${posting.posting_uploadpath}"
										src="/upload/${posting.posting_uploadpath}" width="200">
									</a>
									<p>
								</c:if>
								<!-- 포스팅 제목 / 포스팅 날짜 -->
						...
						<div class="panel-footer">
							...
							<a href="#" title="좋아요"><span data-name="like"
								data-key="${posting.posting_no}"
								class="glyphicon glyphicon-${hearticon}" aria-hidden="true"></span></a>
							좋아요 <span data-name="likeCnt" data-key="${posting.posting_no}">${posting.posting_likecount}</span>개
							<!-- 댓글 -->
							<a href="#comment_${posting.posting_no}" data-toggle="collapse"
								title="댓글"><span class="glyphicon glyphicon-comment"
							...
							<a href="#" title="보관"><span data-name="save"
								data-key="${posting.posting_no}"
								class="glyphicon glyphicon-${saveicon}" aria-hidden="true"></span></a>
							<!-- 신고 -->
							<a href="#" title="신고"><span
								class="glyphicon glyphicon-filter" aria-hidden="true"></span></a>
							...
							<div id="comment_${posting.posting_no}" class="collapse">
								...
								<form action="" id="frm_comment_${posting.posting_no}">
									<input type="hidden" name="user_no"
										value="${sessionScope.member.user_no}"> <input
										type="hidden" name="posting_no" value="${posting.posting_no}">
									${sessionScope.member.user_nickname} : <input type="text"
										name="comment_content"> <a href="#" title="댓글"><span
										data-name="comment" data-key="${posting.posting_no}">댓글</span></a>
								</form>
								<!-- 댓글리스트 -->
								<c:forEach var="comments" items="${posting.comments}"
									varStatus="cmtStat">
									<!-- 댓글내용 -->
									<span id="cmt_${comments.comment_no}"> <span>${comments.user_nickname}
											: ${comments.comment_content}</span> <a href="#" title="삭제"><span
											data-name="comment_delete" data-key="${comments.comment_no}"
											class="glyphicon glyphicon-remove" aria-hidden="true"></span></a><br>
									</span>
								</c:forEach>
							</div>
						</div>
					...
					<div class="modal-dialog">
						...
						<div class="modal-content">
							...
							<div class="modal-body">
								<p>
									<img alt="${posting.posting_uploadpath}"
										src="/upload/${posting.posting_uploadpath}" width="568">
								</p>
								<!-- 포스팅 제목 / 포스팅 날짜 -->
								${posting.posting_title}
								<fmt:formatDate pattern="yyyy-MM-dd HH:mm"
	   		...
	   		#${hashtag.tag}&nbsp;
							...
							<div class="modal-footer">
								<div style="text-align: left;">
									...
									좋아요 <span data-name="likeCnt" data-key="${posting.posting_no}">${posting.posting_likecount}</span>개
									<!-- 댓글 -->
									<a href="#modal_comment_${posting.posting_no}"
										data-toggle="collapse" title="댓글"><span
									...
									<div id="modal_comment_${posting.posting_no}" class="collapse">
										...
										<form action="" id="frm_comment_${posting.posting_no}_modal">
											<input type="hidden" name="user_no"
												value="${sessionScope.member.user_no}"> <input
												type="hidden" name="posting_no"
												value="${posting.posting_no}">
											${sessionScope.member.user_nickname} : <input type="text"
												name="comment_content"> <a href="#" title="댓글"><span
												data-name="comment" data-type="modal"
												data-key="${posting.posting_no}">댓글</span></a>
										</form>
										<!-- 댓글리스트 -->
										<c:forEach var="comments" items="${posting.comments}"
											varStatus="cmtStat">
											<!-- 댓글내용 -->
											<span id="modal_cmt_${comments.comment_no}"> <span>${comments.user_nickname}
													: ${comments.comment_content}</span> <a href="#" title="삭제"><span
													data-name="comment_delete"
													data-key="${comments.comment_no}"
													class="glyphicon glyphicon-remove" aria-hidden="true"></span></a><br>
											</span>
										</c:forEach>
									</div>
